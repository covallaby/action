...
![coverage](docs/coverage-badge.svg) — generated by Covallaby itself on every push.
...
**Seeing our own coverage:** the badge above is committed back to the repo by
CI whenever the number changes; the full HTML report is the
`covallaby-report` artifact on every [workflow run](../../actions), and will
be hosted at `covallaby.github.io/covallaby` once the repo is public
